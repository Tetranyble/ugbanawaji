  ...
  return (
    <div className="max-w-7xl">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <Card className="shadow-none"><CardHeader><CardTitle>Popular questions</CardTitle><CardDescription>Repeated questions reveal what visitors most want the portfolio to explain.</CardDescription></CardHeader><CardContent className="space-y-3">{popular.length ? popular.map((item) => <div key={normalizeQuestion(item.question)} className="flex items-start justify-between gap-4 rounded-xl border border-border p-4"><p className="text-sm leading-6">{item.question}</p><Badge>{item.count}×</Badge></div>) : <p className="text-sm text-muted-foreground">No visitor questions yet.</p>}</CardContent></Card>
      </div>
