...
import { db } from "@/db";
import { availabilityProfiles, availabilityTargetRoles, availabilityWorkModes, resumeVariants } from "@/db/schema";
...
export default async function HireAdminPage() {
  const [[availabilityRow], targetRoles, workModes, resumes] = await Promise.all([
    db.select().from(availabilityProfiles).limit(1),
    db.select().from(availabilityTargetRoles).orderBy(desc(availabilityTargetRoles.sortOrder)),
    db.select().from(availabilityWorkModes).orderBy(desc(availabilityWorkModes.sortOrder)),
    db.select().from(resumeVariants).orderBy(desc(resumeVariants.isDefault), asc(resumeVariants.sortOrder), asc(resumeVariants.name)),
  ]);
  const availability = {
    visible: availabilityRow?.visible ?? false,
    status: availabilityRow?.status ?? "",
    relocation: availabilityRow?.relocation ?? "",
    note: availabilityRow?.note ?? "",
    targetRoles: targetRoles.map((item) => item.label),
    workModes: workModes.map((item) => item.label),
  };
    ...
    <div className="max-w-6xl">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,.85fr)]">
        <Card className="shadow-none">
          ...
          <CardContent><AvailabilityForm availability={availability} action={saveAvailabilitySettings} /></CardContent>
        </Card>
        <Card className="h-fit shadow-none">
          ...
          <CardContent className="space-y-4 text-sm leading-6 text-muted-foreground">
            ...
            <p className="rounded-xl bg-muted p-4">The public /hire page combines these variants with your current focus areas and working stack.</p>
          </CardContent>
        </Card>
      </div>
      ...
      <section className="mt-10">
        ...
        {resumes.length ? <div className="grid gap-4 md:grid-cols-2">{resumes.map((resume) => <Card key={resume.id} className="shadow-none"><CardContent className="p-5"><div className="flex flex-col items-start gap-4 sm:flex-row sm:justify-between"><div className="w-full min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge>{resume.status === "PUBLISHED" ? "Published" : "Draft"}</Badge>{resume.isDefault ? <Badge className="border-primary/25 text-primary">Default</Badge> : null}</div><h3 className="mt-3 break-words font-extrabold">{resume.name}</h3><p className="mt-1 break-words text-sm text-muted-foreground">{resume.targetRole}</p>{resume.summary ? <p className="mt-3 line-clamp-3 text-sm leading-6 text-muted-foreground">{resume.summary}</p> : null}</div><Button asChild size="sm" variant="outline" className="shrink-0"><Link href={`/admin/hire/resumes/${resume.id}`}><Pencil className="size-3.5" /> Edit</Link></Button></div></CardContent></Card>)}</div> : <Card className="border-dashed shadow-none"><CardContent className="flex flex-col items-center py-12 text-center"><FileText className="size-6 text-primary" /><h3 className="mt-3 font-bold">No résumé variants yet</h3><p className="mt-2 max-w-md text-sm text-muted-foreground">Add role-specific versions when they materially improve your positioning.</p><Button asChild className="mt-5"><Link href="/admin/hire/resumes/new"><Plus className="size-4" /> Add first variant</Link></Button></CardContent></Card>}
      </section>
    </div>
  );
}
