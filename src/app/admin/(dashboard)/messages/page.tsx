  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="space-y-5">
        {items.map((message) => <Card key={message.id} className="shadow-none"><CardHeader><div className="flex min-w-0 flex-col items-start gap-4 sm:flex-row sm:justify-between"><div className="w-full min-w-0"><div className="flex min-w-0 items-start gap-2"><MessageSquareText className="mt-1 size-4 shrink-0 text-primary" /><CardTitle className="text-xl">{message.subject || "Portfolio enquiry"}</CardTitle></div><p className="mt-2 break-words text-sm text-muted-foreground">{message.name} · <a className="break-all hover:text-primary" href={`mailto:${message.email}`}>{message.email}</a> · {formatDate(message.createdAt)}</p></div><Badge className="shrink-0">{message.status}</Badge></div></CardHeader><CardContent><p className="whitespace-pre-wrap break-words rounded-xl bg-muted/40 p-4 leading-7 text-muted-foreground">{message.message}</p><ContactCrmForm status={message.status} internalNotes={message.internalNotes} action={updateContactCrm.bind(null, message.id)} /></CardContent></Card>)}
