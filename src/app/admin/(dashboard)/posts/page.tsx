...
export default async function AdminPostsPage() {
  ...
  const now = new Date();
  return <div className="max-w-6xl"><div className="flex items-end justify-between gap-5"><div><p className="section-kicker">Writing</p><h1 className="mt-2 text-3xl font-extrabold">Posts</h1><p className="mt-2 text-muted-foreground">Draft, schedule and publish technical writing.</p></div><Button asChild><Link href="/admin/posts/new"><Plus className="size-4" /> New post</Link></Button></div><div className="mt-8 space-y-4">{items.map((post) => {
