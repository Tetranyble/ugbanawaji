...
import { getPublishedProject, getPublicProfile } from "@/lib/data";
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [project, profile] = await Promise.all([getPublishedProject(slug), getPublicProfile()]);
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 64, background: "#0b1020", color: "white", fontFamily: "sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 18, fontSize: 28, fontWeight: 700 }}>
        ...
        <span style={{ color: "#94a3b8", fontSize: 22 }}>· Case study</span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        <div style={{ fontSize: 58, fontWeight: 800, lineHeight: 1.05, letterSpacing: "-0.03em", maxWidth: 1040 }}>{project?.title ?? "Engineering Case Study"}</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 22, color: "#cbd5e1" }}>
          <span>ugbanawaji.com</span>
          <span>{project?.kind ?? "Software Engineering"}</span>
        </div>
      </div>
    </div>,
    size,
